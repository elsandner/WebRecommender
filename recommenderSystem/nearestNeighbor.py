...
class NearestNeighbor:
    ...
    def getRecommendation(self, userId):    # like main in assignment3
        ...
        #TODO: transfer topMovies format to a list of Movies
        topMoviesList = list(topMovies.keys())
...
def task_3C(rated_movies, dataframeMovies):
    ...
    sampled_movies = dataframeMovies[["title", "genres"]].sample(frac=0.10, random_state=1)
    # modify genres column from "[{'id': 18, 'name': 'Drama'}]" to "Drama" for better readability
    ...
    topMovies = res = dict(list(movie_score_dict.items())[0: 20])  # get first 20 Movies
    print("\n\nTop 10 Recomended Movies:")
    for key, value in topMovies.items():
        print(key + "    \nScore: " + str(value) + "\n")
    return topMovies
...
def calculate_score(rated_movies,sampled_movies,k):
    ...
    for index, new_movie in sampled_movies.iterrows():
        movie_score_dict[new_movie["title"]] = kNN(rated_movies,new_movie,k) #check only the first k movies for the
        count += 1
        if count % 1000 == 0:
